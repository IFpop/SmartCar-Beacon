...
char txt5[50];
static struct vector3f_t stop_vector = {0.0f, 0.0f, 0.0f};
 ...
 * @retval 向量
 */
static struct vector3f_t* speed2vector(uint8_t* speed_vector, uint16_t speed){
	...
	control_vector.y = trans_vec[4]*scale[0]+trans_vec[5]*scale[1]+trans_vec[6]*scale[2]+trans_vec[3]*scale[7];
	control_vector.y = trans_vec[8]*scale[0]+trans_vec[9]*scale[1]+trans_vec[10]*scale[2]+trans_vec[11]*scale[7];
...
void speed_control(void){
	...
	ANO_DT_send_int16(0, 0, 0,0, enc1, enc2, enc3, enc4);
...
//	ANO_DT_send_int16(motor1_pluse, motor2_pluse, motor1Last,motor2Last, enc1, enc2, enc3, enc4);
}
