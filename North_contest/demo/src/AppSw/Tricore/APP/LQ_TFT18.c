...
void lq_tft_short_delay(unsigned int Del)	//
{
  while(Del--)
  {
	  __asm("NOP");
...
void TFTSPI_Write_Cmd(unsigned char cmd)
{
  ...
  for(i=0;i<8;i++)
  {
    TFTSPI_SCK_L;lq_tft_short_delay(1);	// SCK=0  ILI9163_SCK=0;
    if(cmd&0x80){TFTSPI_SDI_H;}// SDI=1
    else{TFTSPI_SDI_L;}// SDI=0
    TFTSPI_SCK_H;lq_tft_short_delay(1);	// SCK=1  ILI9163_SCK = 1;
    cmd = (cmd<<1);
  }
}
...
*/
void TFTSPI_Show_Line(unsigned char xs,unsigned char ys,unsigned short color)
{
  unsigned int i,j;
...
入口参数：起始、终止横坐标(0-127)，纵坐标(0-159),显示颜色unsigned short
出口参数：无
时间：2012-09-09
功能说明：填充矩形区域
其他说明：0<=xs<xe<=127
...
入口参数：圆心横坐标(0-127)，纵坐标(0-159),半径(0-128),显示颜色unsigned short
出口参数：无
时间：2012-09-09
功能说明：画圆形边框(仅支持屏幕内画圆)
其他说明：0<=x<=127
...
void TFTSPI_Draw_Line(unsigned char xs,unsigned char ys,unsigned char xe,unsigned char ye,unsigned short color_dat)
{
  ...
  else								//如果是斜线，则重新计算，使用画点函数画出直线
  {
    ...
    if(dx > dy) ds = (unsigned char)dx;					//选取基本增量坐标轴
    else		ds = (unsigned char)dy;
...
入口参数：起始横坐标(0-127)，纵坐标(0-159),显示颜色unsigned short
出口参数：无
时间：2012-09-09
功能说明：画点
其他说明：0<=x<=127
...
void TFTSPI_Show_Pic(unsigned char xs,unsigned char ys,unsigned char xe,unsigned char ye,unsigned char *ppic)
{
...
void TFTSPI_Show_pic()
{
  ...
  */	
  ...
  { 
    for(j=0;j<TFT18W;j++)
    {			
      //TFTSPI_Write_Word(((unsigned short)MYPIC[k])<<8|MYPIC[k+1]);
      TFTSPI_Write_Word(((unsigned short)u16ftflogo[k])<<8|u16ftflogo[k+1]);
...
//x: 0 - 7	(行)
//y: 0 -12	(列)
void TFTSPI_P16x12Str(unsigned char x,unsigned char y,char *s_dat,unsigned short word_color,unsigned short back_color)
{
...
//num: 0 - 65535	要显示的数字
//num_bit: 0 - 5	数字的位数
void TFTSPI_P6X8NUM(unsigned char x, unsigned char y, unsigned short num, unsigned char num_bit,unsigned short word_color,unsigned short back_color)
{
  char i;
  //unsigned char ii;
  unsigned char dat[6];
...
//num: 0 - 65535	要显示的数字
//num_bit: 0 - 5	数字的位数
void TFTSPI_P8X8NUM(unsigned char x, unsigned char y, unsigned short num, unsigned char num_bit,unsigned short word_color,unsigned short back_color)
{
  char i;
  unsigned char ii = num_bit;
  unsigned char dat[6];
  for(i = 0; i < 6; i++) dat[i] = 0; i = 0;	//初始化数据
  while(ii--)								//拆位
  {
...
unsigned char ColumnarMAX=5;
unsigned char Start_X,Start_Y;
unsigned char AreaW,AreaH;
unsigned char Colum_W,Colum_H;
unsigned short BColor,FColor;

//  起始点x,起始点y,频谱宽度，高度，前景色，背景色
void ColumnarSetting(unsigned char x,unsigned char y, unsigned char w,unsigned char h,unsigned short fclolr,unsigned short bcolor)
{
  Start_X=x;
  Start_Y=y;
...
void DrawingColumnar(unsigned char t,unsigned char duty,unsigned char cl)
{
  unsigned int i,j;  
  
  //上半截
  TFTSPI_Set_Pos(Start_X+Colum_W*t,Start_Y,Start_X+Colum_W*(t+1),(unsigned char)(Start_Y+Colum_H*(1-duty*0.01)));
  ...
  //下半截 
  TFTSPI_Set_Pos(Start_X+Colum_W*t,(unsigned char)(Start_Y+Colum_H*(1-duty*0.01)),Start_X+Colum_W*(t+1),Start_Y+Colum_H);
  for(i=0;i<Colum_W+2;i++)      //160
  {
    for(j=0;j<Colum_H+2;j++)
    {
...
{
    TFTSPI_Show_Pic2(xs,ys,160,54,lqlogo);       //80,gImage_ftflogo);     //
}
...
void TFT_Full_Scr_Road(void)
{
  ...
  TFTSPI_Set_Pos(0,0,159,119);

  for(i=0;i<120;i++)    //行
  {
    for(j=14;j<174;j++)  //列
    {
//#ifdef __USE_GRAY8BIT  //8位灰度显示
      TFTSPI_Write_Word(((Image_Data[i][j]>>4)<<11)|((Image_Data[i][j]>>3)<<5)|(Image_Data[i][j]>>4));
//#else                  //二值显示
//      if(Image_Use[i][j] >Threshold)  TFTSPI_Write_Word(u16BLACK); //16位像素值
//      else              TFTSPI_Write_Word(u16WHITE);
//#endif
    }
  }
}
